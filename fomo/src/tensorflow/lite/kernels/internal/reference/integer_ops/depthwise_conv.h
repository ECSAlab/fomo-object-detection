...
#if 0
inline void DepthwiseConvPerChannelSW(
...
inline void DepthwiseConvPerChannelHW(
    const DepthwiseParams& params, const int32_t* output_multiplier,
    const int32_t* output_shift, const RuntimeShape& input_shape,
    const int8_t* input_data, const RuntimeShape& filter_shape,
    const int8_t* filter_data, const RuntimeShape& bias_shape,
        ...
        for (int d = 0; d < input_depth; ++d) 
        {
            ...
            for (int filter_y = 0; filter_y < filter_height; ++filter_y) 
            {
              ...
              for (int filter_x = 0; filter_x < filter_width; ++filter_x) 
              {
                ...
                {
                  int32_t input_val = input_data[Offset(input_shape, 0, in_y, in_x, d)];
                  int32_t filter_val = filter_data[Offset(filter_shape, 0, filter_y, filter_x, d)];
      ...
      for (int w = 0; w < output_width; ++w) 
      {
        for (int d = 0; d < input_depth; ++d) 
        {
            ...
            for (int filter_y = 0; filter_y < filter_height; ++filter_y) 
            {
              int in_x = w * 2;
              int32_t input_val = input_data[Offset(input_shape, 0, in_y, in_x, d)];
              int32_t filter_val = filter_data[Offset(filter_shape, 0, filter_y, 0, d)];
              acc += filter_val * (input_val + input_offset);
              
              input_val = input_data[Offset(input_shape, 0, in_y, in_x + 1, d)];
              filter_val = filter_data[Offset(filter_shape, 0, filter_y, 1, d)];
              acc += filter_val * (input_val + input_offset);
              
              input_val = input_data[Offset(input_shape, 0, in_y, in_x + 2, d)];
              filter_val = filter_data[Offset(filter_shape, 0, filter_y, 2, d)];
              acc += filter_val * (input_val + input_offset);
              in_y++;
            }
            acc += bias_data[d];
...
inline void acc3x3(int h,int w,int d,
                  int8_t* output_data,const RuntimeShape& output_shape,
...
{ 
  ...
  cfu_quant_init(output_multiplier[d], -output_shift[d]);
    ...
    int8_t input_val = input_data[Offset(input_shape, 0, in_y, in_x, d)];
    int8_t filter_val = filter_data[Offset(filter_shape, 0, 0, 0, d)];
    cfu_acc(input_val,filter_val);
    input_val = input_data[Offset(input_shape, 0, in_y, in_x + 1, d)];
    filter_val = filter_data[Offset(filter_shape, 0, 0, 1, d)];
    cfu_acc(input_val,filter_val);
    
    input_val = input_data[Offset(input_shape, 0, in_y, in_x + 2, d)];
    filter_val = filter_data[Offset(filter_shape, 0, 0, 2, d)];
    cfu_acc(input_val,filter_val);
    in_y++;
    in_x = w * 2;
    input_val = input_data[Offset(input_shape, 0, in_y, in_x, d)];
    filter_val = filter_data[Offset(filter_shape, 0, 1, 0, d)];
    cfu_acc(input_val,filter_val);
    
    input_val = input_data[Offset(input_shape, 0, in_y, in_x + 1, d)];
    filter_val = filter_data[Offset(filter_shape, 0, 1, 1, d)];
    cfu_acc(input_val,filter_val);
    input_val = input_data[Offset(input_shape, 0, in_y, in_x + 2, d)];
    filter_val = filter_data[Offset(filter_shape, 0, 1, 2, d)];
    cfu_acc(input_val,filter_val);
    in_y++;
    in_x = w * 2;
    input_val = input_data[Offset(input_shape, 0, in_y, in_x, d)];
    filter_val = filter_data[Offset(filter_shape, 0, 2, 0, d)];
    cfu_acc(input_val,filter_val);
    
    input_val = input_data[Offset(input_shape, 0, in_y, in_x + 1, d)];
    filter_val = filter_data[Offset(filter_shape, 0, 2, 1, d)];
    cfu_acc(input_val,filter_val);
    
    input_val = input_data[Offset(input_shape, 0, in_y, in_x + 2, d)];
    filter_val = filter_data[Offset(filter_shape, 0, 2, 2, d)];
    cfu_acc(input_val,filter_val);
          ...
          for (int d = 0; d < input_depth; ++d) 
          {
              ...
              for (int filter_y = 0; filter_y < filter_height; ++filter_y) 
              {
                ...
                for (int filter_x = 0; filter_x < filter_width; ++filter_x) 
                {
                  ...
                  {
                    int8_t input_val  = input_data[Offset(input_shape, 0, in_y, in_x, d)];
                    int8_t filter_val = filter_data[Offset(filter_shape, 0, filter_y, filter_x, d)];
                    cfu_acc_simd(input_val,filter_val);
        ...
        for (int w = 0; w < output_width; ++w) 
        {
          for (int d = 0; d < input_depth; ++d) 
          {
              ...
              for (int filter_y = 0; filter_y < filter_height; ++filter_y) 
              {
                int in_x = w * 2;
                int8_t input_val = input_data[Offset(input_shape, 0, in_y, in_x, d)];
                int8_t filter_val = filter_data[Offset(filter_shape, 0, filter_y, 0, d)];
                cfu_acc(input_val,filter_val);
...
inline void DepthwiseConvPerChannel(
    ...
    const int32_t* bias_data, const RuntimeShape& output_shape,
    int8_t* output_data) {
  #ifdef SHOW_DEPTHWISECONV_PARAMS
  // Format is:
  ...
  #endif
  #ifdef ACCEL_DEPTHWISECONV
    #ifdef ACCEL_DEPTHWISECONV_HW
      DepthwiseConvPerChannelHW(params, output_multiplier, output_shift, input_shape,
                      input_data, filter_shape, filter_data, bias_shape,
                      bias_data, output_shape, output_data);
    #else
      DepthwiseConvPerChannelSW(params, output_multiplier, output_shift, input_shape,
                      input_data, filter_shape, filter_data, bias_shape,
                      bias_data, output_shape, output_data);
    #endif 
