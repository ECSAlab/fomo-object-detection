...
sfl_test_req  = b"starttest\n"
sfl_test_ack  = b"testack\n"
...
class LiteXTerm:
    def __init__(self, serial_boot, kernel_image, kernel_address, json_images, safe):
        ...
        self.test_detect_buffer   = bytes(len(sfl_test_req))
    ...
    def detect_test(self, data):
        if len(data):
            self.test_detect_buffer = self.test_detect_buffer[1:] + data
            return self.test_detect_buffer == sfl_test_req
        else:
            return False

    def answer_test(self):
        
        print("[LITEX-TERM] Test Start on device.")
        i = 0 
        start = time.time()

        for filename in os.listdir(os.getcwd()+"/utils/img"):
            # Show progress
            i = i + 1
            sys.stdout.write(" Sending image: {}\r".format(i))
            print( i,":",filename)
           
            sys.stdout.flush()
            with open(os.path.join(os.getcwd()+"/utils/img/", filename), 'rb') as f: # open in readonly mode
                data = f.read()
                self.port.write(data)
                f.close()
                time.sleep(0.70) # Sleep for 
        
        
        end = time.time()
        elapsed = end - start
        print("[LITEX-TERM] Test End in {} seconds",elapsed)
